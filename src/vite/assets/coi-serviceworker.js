...
if (typeof self !== 'undefined' && typeof window === 'undefined') {
  ...
  self.addEventListener('fetch', (event) => {
    ...
    // Range requests replayed from the cache must be left alone or the browser errors before the response is ever read.
    if (request.cache === 'only-if-cached' && request.mode !== 'same-origin') return

    event.respondWith(
      fetch(request)
        .then((response) => {
          // Opaque responses have no readable headers and cannot be rebuilt; `credentialless` lets them through as they are.
  ...
  // `document.currentScript` is only readable while this script is executing, so the URL is captured now, not inside the promise.
  const workerUrl = document.currentScript && document.currentScript.src
  const reloadedKey = 'fkn-container-coi-reloaded'

  const reloadOnce = () => {
    // Guards a reload loop when isolation is unreachable at all: not a secure context, or an unknown COEP value.
