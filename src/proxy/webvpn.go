...
// The imports never await on the JS side: they poke/drain per-socket ring buffers that JS fills while this worker is parked in Atomics.wait between calls.
...
// All imports return 0 on success or a non-zero errno-ish code, write output values through the *P pointer arguments, and recv flags bit0 = EOF.
...
// Callers must runtime.KeepAlive the backing object across the import call so the GC can't move/free it mid-call.
...
// Writes never block on a timeout in this model, so the write deadline is a no-op.
