...
function connect(name, shared, conn, certbuf) {
    ...
    // c2w-webvpn: lazily build a webvpn netstack the first time a webvpn_*
    // message arrives. Constructed here on the main thread because @webvpn
    // sockets must live where their async callbacks can run.
    var webvpn;
    function ensureWebvpn() {
    ...
    return function(msg){
        ...
        if (typeof req_ == "object" && req_.type) {
            // First refusal for c2w-webvpn egress messages; falls through to the
